namespace Datapack
{
    ...
    public class GameFunctionEvent
    {
        ...
        public class LocalPlayer
        {
            ...
            internal void Effect(bool give = true, Effect effect = Components.Effect.Speed, uint dur = 30, byte amp = 0, bool hide = false)
            {
                EffectComponent effectComponent = Component.Effect(effect, dur, amp, hide);
                _ctx._output.AppendLine($"effect {(give == true ? "give" : "clear")} " + $"@s {(give == true ? effectComponent.ToRaw() : effectComponent.GetEffect().ToString().ToLower())}");
            }
        }
        ...
        internal void Effect(string target, bool give = true, Effect effect = Components.Effect.Speed, uint dur = 30, byte amp = 0, bool hide = false)
        {
            EffectComponent effectComponent = Component.Effect(effect, dur, amp, hide);
            _output.AppendLine($"effect {(give == true ? "give" : "clear")} " +
